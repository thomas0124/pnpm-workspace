...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({ className, size = "medium", color, variant, ...props }, ref) => {
  const sizeClass = sizeClasses[size];
  const colorClass = color
    ? (variant === "outline" ? colorClasses[color].outline : colorClasses[color].default)
    : "";
  return (
    <BaseButton ref={ref} className={cn("w-full rounded-lg font-medium", sizeClass, colorClass, className)} {...props} />
  );
});
