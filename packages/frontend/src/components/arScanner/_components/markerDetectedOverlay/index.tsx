...
export function MarkerDetectedOverlay() {
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      loader.load("/fonts/helvetiker_bold.typeface.json", (font) => { // publicフォルダからのパス
  ...
  return (
    <div className="absolute left-1/2 top-1/2 flex w-full max-w-md -translate-x-1/2 -translate-y-1/2 flex-col items-center">
      ...
      <div ref={containerRef} className="h-96 w-full animate-in fade-in zoom-in duration-300" />
      
      {/* 情報表示部分（元のコンポーネントから維持） */}
      <div className="mt-4 rounded-2xl bg-white/90 p-6 shadow-2xl backdrop-blur-sm">
        <h2 className="mb-2 text-center text-lg font-bold text-gray-900">AR MARKER DETECTED</h2>
        <p className="text-center text-sm text-gray-500">Custom Shader Object</p>
      </div>
    </div>
  )
}